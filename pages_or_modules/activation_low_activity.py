...
def _apply_geo_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if scope == "MIXED":
        selected_city = multiselect_filter("City", df, "geo_city", key="act_geo_city")
...
def _apply_nz_geo_filters(df: pd.DataFrame, *, selected_city: list[str] | None = None) -> pd.DataFrame:
    ...
    if selected_city is None:
        selected_city = multiselect_filter("City", filtered, "geo_city", key="act_geo_city")
        if selected_city:
    ...
    for label, column, key in (
        ("NZ cluster", "nz_business_cluster", "act_nz_cluster"),
        ("Suburb", "geo_suburb", "act_geo_suburb"),
    ):
        selected = multiselect_filter(label, filtered, column, key=key)
        if selected:
            filtered = filtered[filtered[column].astype(str).isin(selected)]
